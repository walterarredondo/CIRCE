...
server: $(SRC_DIR)/server.c $(INCLUDE_DIR)/server.h  $(INCLUDE_DIR)/json_utils.h  $(INCLUDE_DIR)/connection.h $(INCLUDE_DIR)/logger.h
	gcc -Wall -Wextra -Wno-unused-variable  -Wno-unused-parameter  -std=c11 -O2 -o $(BIN_DIR)/server $(SRC_DIR)/server.c $(SRC_DIR)/json_utils.c $(SRC_DIR)/connection.c $(SRC_DIR)/logger.c -I$(INCLUDE_DIR) $(CFLAGS) $(LDLIBS) -lglib-2.0 -ljansson -lpthread 

client: $(SRC_DIR)/client.c $(INCLUDE_DIR)/client.h  $(INCLUDE_DIR)/json_utils.h $(INCLUDE_DIR)/connection.h $(INCLUDE_DIR)/logger.h $(INCLUDE_DIR)/client.h 
	gcc -Wall -Wextra -Wno-unused-variable -Wno-unused-parameter   -std=c11 -O2 -o $(BIN_DIR)/client $(SRC_DIR)/client.c -I$(INCLUDE_DIR) $(SRC_DIR)/json_utils.c $(SRC_DIR)/connection.c  $(SRC_DIR)/logger.c $(CFLAGS) $(LDLIBS) -ljansson -lpthread
